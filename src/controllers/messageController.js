...
export const sendMessage = async (req, res) => {
  try {
    ...
    if (!receiver_username || !text) {
      return res.status(400).json({ error: 'Receiver username or phone and text are required' });
    }
    ...
    // 🔍 STEP 1: Check if receiver is a registered user (by username OR phone)
    const receiver = await pool.query(
      'SELECT id, username, phone FROM users WHERE username = $1 OR phone = $1',
      [receiver_username]
    );
    ...
    // ❌ CASE 2: User is NOT REGISTERED - treat as PHONE NUMBER
    console.log(`📞 User not registered. Treating as phone number: ${receiver_username}`);

    // Store message in pending_invites table
    const pendingMessage = await pool.query(
      ...
      [sender_id, receiver_username, text, false]
    );

    console.log(`✅ Message saved to pending_invites (ID: ${pendingMessage.rows[0].id})`);

    // 📲 STEP 2: Send SMS notification via AWS SNS
    const messagePreview = text.length > 50 ? text.substring(0, 50) : text;
    const smsResult = await sendSMSNotification(receiver_username, sender_name, messagePreview);
    ...
    return res.status(201).json({
      success: true,
      message: 'User not registered. Message saved and SMS notification sent.',
      pending_invite_id: pendingMessage.rows[0].id,
